  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-text">Task Review</h1>
          <p className="mt-0.5 text-sm text-muted">Quality rubric, comments & review queue</p>
        </div>
        <button
          type="button"
          onClick={load}
              ...
              <button
                ...
                className={`w-full rounded-xl border p-3 text-left transition-colors ${
                  selected?.id === item.id
                    ...
                    : "border-border bg-card hover:border-border-hover hover:bg-elevated"
                }`}
              >
                <p className="truncate text-sm font-medium text-text">{item.title}</p>
                <p className="text-xs text-muted">
                  {item.user?.name} - {item.project?.code || "-"}
                </p>
                {item.qualityScore != null && (
                  <span className="mt-1 inline-block rounded-md bg-success/10 px-1.5 py-0.5 text-xs font-medium text-success">
          ...
          {selected ? (
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              {canReview && filter !== "reviewed" && (
                <>
                  <p className="mb-2 mt-6 text-xs font-semibold uppercase tracking-wider text-muted">Quality Rubric</p>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {RUBRIC.map((r) => (
                      <button
                        ...
                        className="rounded-lg border border-border bg-surface p-3 text-left transition-colors hover:border-accent hover:bg-accent/5"
                      >
                        <div className="flex items-center gap-2">
                          <Star size={13} className="text-accent" />
                          <span className="font-semibold text-text">{r.score}%</span>
                  ...
                  <div className="mt-4">
                    ...
                    <textarea
                      ...
                      className="w-full rounded-lg border border-border bg-surface px-3.5 py-2.5 text-sm text-text outline-none transition-colors placeholder:text-muted hover:border-border-hover focus:border-accent focus:ring-1 focus:ring-accent/30"
                      rows={3}
                    />
                  </div>
                </>
